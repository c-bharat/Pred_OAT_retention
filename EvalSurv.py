...
class EvalSurv:
    ...
    def integrated_brier_score(self, times=None):
        if self.cens_surv is None:
            self.compute_cens_surv()
        if times is None:
            times = self.df['TIME'].max()
        self.cens_surv['BRIER'] = 0.
        for index, row in self.cens_surv.iterrows():
            self.cens_surv.at[index, 'BRIER'] = self.brier(index)
        cens_surv_ibs = self.cens_surv[self.cens_surv.index <= times]
        # integrate
        print('IBS calculated over range:', cens_surv_ibs.index.min(), 'to', cens_surv_ibs.index.max())
        total_time = cens_surv_ibs.index.max() - cens_surv_ibs.index.min()
        ibs = np.trapz(cens_surv_ibs['BRIER'], cens_surv_ibs.index) / total_time
        return ibs
