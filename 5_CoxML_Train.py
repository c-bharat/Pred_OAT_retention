...
def main():
    ...
    for fold in range(5):
        for swap in range(2):
            ...
            codes, month, diagt = codes[sort_idx], weeks[sort_idx], diagt[sort_idx]

            # Standardise time variable based on training data
            time = StandardScaler().fit_transform(time.reshape(-1, 1)).flatten()

            print('Create data loaders and tensors for fold data...')
            ...
            for trial in range(hp.num_trials):
                ...
                print('Train...')
                # Neural Net
                model_name = 'ml_mod_fold_' + str(fold) + '_swp_' + str(swap) + '_trial_' + str(trial) + '.pt'
                ...
                os.makedirs(hp.log_dir + 'fold_' + str(fold) + '_' + str(swap) + '/', exist_ok=True)
                torch.save(net.state_dict(), hp.log_dir + 'fold_' + str(fold) + '_' + str(swap) + '/' + model_name)
                print('Done')
